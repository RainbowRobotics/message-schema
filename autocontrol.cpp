...
bool AUTOCONTROL::is_state_valid(const ompl::base::State *state) const
{
    ...
    if(obsmap->is_tf_collision_with_area(robot_tf, avoid_area))
    {
        // is collision        
        return false;
    }
...
int AUTOCONTROL::get_valid_idx(std::vector<Eigen::Matrix4d>& path, int st_idx)
{
    ...
    {
        if(!obsmap->is_tf_collision(path[p], 0.2, 0.05))
        {
            return p;
        }
    }
...
PATH AUTOCONTROL::calc_local_path()
{
    ...
    int st_idx = std::min<int>(cur_idx+3, (int)global_path.pos.size()-1);
...
PATH AUTOCONTROL::calc_avoid_path()
{
    ...
    // calc avoid area
    avoid_area = obsmap->calc_avoid_area(global_path.pose);
    ...
    // set state space
    auto stateSpace(std::make_shared<ompl::base::DubinsStateSpace>(0.0001, false));
    ...
    if(obsmap->is_tf_collision(ed_tf))
    {
        bool is_found = false;
        double min_d = 99999999;
        ...
        for(double dx = -0.25; dx <= 0.25; dx += 0.05)
        {
            for(double dy = -0.25; dy <= 0.25; dy += 0.05)
            {
                ...
                if(!obsmap->is_tf_collision(tf) && d < min_d)
                {
                    min_d = d;
                    min_xi = xi;
                    is_found = true;
...
void AUTOCONTROL::b_loop_pp()
{
    ...
    while(b_flag)
    {
        ...
        // calc local path
        if(get_time() - last_local_path_t > 0.3)
        {
        ...
        if(fsm_state == AUTO_FSM_FIRST_ALIGN)
        {
            ...
            // obs check
            std::vector<Eigen::Matrix4d> traj = calc_trajectory(Eigen::Vector3d(cur_vel[0], cur_vel[1], cur_vel[2]), 0.05, 0.3, cur_tf);
        ...
        else if(fsm_state == AUTO_FSM_DRIVING)
        {
            ...
            double w = (v * std::tan(th) * params.DRIVE_A)/config->ROBOT_WHEEL_BASE;
        ...
        else if(fsm_state == AUTO_FSM_OBS)
        {
            ...
            else if(obs_state == 1)
            {
                ...
                if(obsmap->is_path_collision(traj) || get_time() - reverse_st_t > 2.0)
                {
                    mobile->move(0, 0, 0);
                    obs_state = 2;
                    continue;
