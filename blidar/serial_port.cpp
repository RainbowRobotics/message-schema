...
bool Serial_Port::getTermios(termios *tio) {
  ...
  {
      std::cout << "[FALSE] serial_port::getTermios if (::tcgetattr(fd_, tio) == -1)" << std::endl;
      return false;
  }
...
bool Serial_Port::setBaudrate(unsigned long baudrate) {
  ...
  if (fd_ == -1)
  {
      std::cout << "[FALSE] serial_port::setBaudrate if (fd_ == -1)" << std::endl;
      return false;
  }
...
bool Serial_Port::setTermios(const termios *tio) {
  ...
  if(fcntl(fd_, F_SETFL, FNDELAY))
  {
      std::cout << "[FALSE] serial_port::setTermios if(fcntl(fd_, F_SETFL, FNDELAY))" << std::endl;
      return false;
  }

  if (::tcsetattr(fd_, TCSANOW, tio) == -1)
  {
      std::cout << "[FALSE] serial_port::setTermios if(::tcsetattr(fd_, TCSANOW, tio) == -1)" << std::endl;
      return false;
  }
...
bool Serial_Port::open() {
    ...
    if(port_.empty())
    {
        std::cout << "[FALSE] serial_port::open if (port_.empty())" << std::endl;
        return false;
	}
	...
	*/
    ...
    if(fd_ == -1)
    {
        std::cout << "[FALSE] serial_port::open if (fd_ == -1)" << std::endl;
        return false;
	}
