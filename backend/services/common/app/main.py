...
SERVICE_SIDS: dict[str, str] = {"manipulate": None, "mobility": None, "sensor": None}
...
def route_service_by_event(event: str, data: dict | None) -> str | None:
    for serviceName in SERVICE_SIDS:
...
def parse_auth(auth: dict | None):
    """JWT는 나중에. 지금은 DEV 모드에선 간소화."""
    auth = auth or {}
    # if not DEV_MODE:
...
class RelayNS(socketio.AsyncNamespace):
    ...
    async def on_disconnect(self, sid):
        ...
        if claims.get("role") == "service":
            ...
            if SERVICE_SIDS.get(svc) == sid:
                SERVICE_SIDS[svc] = None
