...
@config_router.post("/{robot_model}/save_robot_code", response_model=Response_FunctionsPD)
...
async def save_user_frame_parameter(robot_model: str, request: Request_Save_User_FramePD):
    return JSONResponse(
        await config_service.save_user_frame_parameter(robot_model=robot_model, request=request)
    )
...
async def set_userframe_6dof(robot_model: str, request: Request_Set_User_Frame_6DofPD):
    return JSONResponse(
        await config_service.set_userframe_6dof(robot_model=robot_model, request=request)
    )
...
async def set_userframe_tcp(robot_model: str, request: Request_Set_User_Frame_TCPPD):
    return JSONResponse(
        await config_service.set_userframe_tcp(robot_model=robot_model, request=request)
    )
...
async def set_userframe_3points(robot_model: str, request: Request_Set_User_Frame_3PointsPD):
    return JSONResponse(
        await config_service.set_userframe_3points(robot_model=robot_model, request=request)
    )
...
async def save_area_parameter(robot_model: str, request: Request_Save_Area_ParameterPD):
    return JSONResponse(
        await config_service.save_area_parameter(robot_model=robot_model, request=request)
    )
...
async def save_direct_teach_sensitivity(
    ...
    return JSONResponse(
        await config_service.save_direct_teach_sensitivity(robot_model=robot_model, request=request)
    )
...
async def save_collision_parameter(robot_model: str, request: Request_Save_Collision_ParameterPD):
    return JSONResponse(
        await config_service.save_collision_parameter(robot_model=robot_model, request=request)
    )
...
async def save_selfcoll_parameter(robot_model: str, request: Request_Save_SelfColl_ParameterPD):
    return JSONResponse(
        await config_service.save_selfcoll_parameter(robot_model=robot_model, request=request)
    )
...
async def set_out_collision_parameter(robot_model: str, request: Request_Set_Out_Collision_ParaPD):
    return JSONResponse(
        await config_service.set_out_collision_para(robot_model=robot_model, request=request)
    )


@config_router.post("/{robot_model}/set_self_collision_para", response_model=Response_ReturnValuePD)
async def set_self_collision_parameter(
    robot_model: str, request: Request_Set_Self_Collision_ParaPD
):
    return JSONResponse(
        await config_service.set_self_collision_para(robot_model=robot_model, request=request)
    )
...
async def save_gravity_parameter(robot_model: str, request: Request_Save_Gravity_ParameterPD):
    return JSONResponse(
        await config_service.save_gravity_parameter(robot_model=robot_model, request=request)
    )


@config_router.post("/{robot_model}/set_shift", response_model=Response_ReturnValuePD)
def set_shift(robot_model: str, request: Request_Set_ShiftPD):
    return config_service.set_shift(robot_model=robot_model, request=request)
...
async def set_joint_impedance(robot_model: str, request: Request_Set_Joint_ImpedancePD):
    return JSONResponse(
        await config_service.set_joint_impedance(robot_model=robot_model, request=request)
    )
...
async def set_freedrive(robot_model: str, request: Request_Set_Free_DrivePD):
    return JSONResponse(
        await config_service.set_freedrive(robot_model=robot_model, request=request)
    )
