...
from app.features.localization.port.database_port import DatabasePort


class LocalizationMongoDatabaseAdapter(DatabasePort):
    """
    [Localization MongoDB 어댑터 초기화]
    """
    ...
    async def check_db(self):
        try:
            ...
            # await mongo_db.db[self.name].drop_index("localization_index")
            ...
            await mongo_db.ensure_index(mongo_db.db, self.name, [
                ...
                ("mapName", "text"),
                ("direction", "text"),("createdAt", -1)], name="localization_index")
            ...
            rb_log.info(f"[LocalizationDatabase] stats: {self.name}: {stats['count']} docs, "
                        f"dataSize={stats['size']}KB, storageSize={stats['storageSize']}KB, "
                        f"indexSize={stats['totalIndexSize']}KB")
        except ServiceException as e:
            raise e
        except Exception as e:
            rb_log.error(f"[LocalizationDatabase] checkDB Error : {e}")
    ...
    async def save(self, command: dict):
        try:
            ...
            rb_log.error(f"[LocalizationDatabase] Save Error : {e}")
    ...
    async def upsert(self, command: dict):
        try:
            ...
            rb_log.error(f"[LocalizationDatabase] Upsert Error : {e}")
    ...
    async def update(self, command: dict):
        try:
            ...
            rb_log.error(f"[LocalizationDatabase] Update Error : {e}")
            raise ServiceException("DB 업데이트 실패", status_code=500) from e
    ...
    async def get_logs(self, options: dict) -> LogsResponse:
        ...
        try:
            ...
            rb_log.info(f"[LocalizationDatabase] getLogs : {options}")
            ...
            print(f"[LocalizationDatabase] getLogs filter_options: {filter_options}, search_text: {search_text}, fields: {fields}, sort: {sort}, order: {order}, page: {page}, limit: {limit}, skip: {skip}")
    ...
    dry_run: bool
        ...
        """
        [Localization DB 로그 아카이브]
        - cutoff_utc: datetime | date # 아카이브 기준 날짜(해당날짜기준부터 이전날짜를 모두 아카이브합니다)
        - dry_run: bool               # 값이 True이면 실제 기능을 수행하진 않고 예상 결과를 반환합니다
        """
        try:
            tz_name = "Asia/Seoul"
            out_dir = "/app/data/amr/archive/localization"
            rb_log.info(f"[LocalizationDatabase] archiveLogs : cutoff_utc: {cutoff_utc}, tz_name: {tz_name}, out_dir: {out_dir}, dry_run: {dry_run}")

            # 1) 날짜 변환 (KST -> UTC)
            cutoff_utc = convert_dt(local=cutoff_utc, in_tz=tz_name, out_tz="UTC")
            rb_log.info(f"[LocalizationDatabase] archiveLogs : {cutoff_utc}")
            ...
            print(f"[LocalizationDatabase] archiveLogs result: {result}")
            return result
        except ServiceException as e:
            print(f"[LocalizationDatabase] archiveLogs ServiceException: {e}")
            raise e
        except Exception as e:
            rb_log.error(f"[LocalizationDatabase] archiveLogs Delete Logs Error : {e}")
    ...
    async def export_logs(self, start_dt: datetime | date, end_dt: datetime | date, filters: dict[str, Any], filename: str, search_text: str, fields: dict[str, Any], sort: str, order: str) -> dict:
        ...
        try:
            ...
            col = mongo_db.db[self.name]
            tz_name = "Asia/Seoul"
            out_dir = "/app/data/amr/export/localization"

            rb_log.info(f"[LocalizationDatabase] exportLogs : {start_dt}, {end_dt}, {filters}, {filename}, {search_text}, {fields}, {sort}, {order}")
            ...
            rb_log.info(f"[LocalizationDatabase] exportLogs start_utc: {start_utc}, end_utc: {end_utc}, filename: {filename}, filters: {filters}, search_text: {search_text}, fields: {fields}, sort: {sort}, order: {order}")
            ...
            rb_log.info("---------------------------------")
            rb_log.info(f"[LocalizationDatabase] exportLogs results: {res}")
            ...
            rb_log.error(f"[LocalizationDatabase] exportLogs ServiceException: {e}")
            raise e
        except Exception as e:
            rb_log.error(f"[LocalizationDatabase] exportLogs Export Logs Error : {e}")
