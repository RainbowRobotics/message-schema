...
def register_zenoh_exception_handlers(app: FastAPI) -> None:
    ...
    async def _no_reply(_: Request, exc: ZenohNoReply):
        return JSONResponse(
            status_code=504,
            content={"detail": "zenoh no reply", "timeout limit": exc.timeout_ms},
        )
    ...
    async def _reply_err(_: Request, exc: ZenohReplyError):
        return JSONResponse(
            status_code=502,
            content={"detail": "peer error", "message": str(exc), "attachment": exc.attachment},
        )
    ...
    async def _transport(_: Request, exc: ZenohTransportError):
        return JSONResponse(
            status_code=502,
            content={"detail": "transport error", "message": str(exc)},
        )
