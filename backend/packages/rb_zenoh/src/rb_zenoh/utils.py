...
def recommend_cap(avg_bytes: float, mem_budget_mb: float, safety: float,
                  min_cap: int = 10, max_cap: int = 200_000) -> int:
    if avg_bytes <= 0:
        avg_bytes = 1.0
...
def resolve_object_api(FBClass) -> Optional[type]:
    """
...
def _find_T_class(root_module, camel_T: str, extra_roots: Optional[list[str]] = None):
    """
