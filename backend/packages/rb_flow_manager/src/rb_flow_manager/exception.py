...
class FlowControlException(Exception):
    """Flow Control 예외"""
class BreakFolder(FlowControlException):
    """현재 Folder(컨테이너) 내부 실행을 중단하고
    Folder 다음 스텝으로 넘어가고 싶을 때 사용."""

class BreakRepeat(FlowControlException):
    """가장 가까운 RepeatStep만 탈출"""

class ContinueRepeat(FlowControlException):
    """다음 내용 무시하고 RepeatStep 다음 부분부터 계속"""

class JumpToStepException(FlowControlException):
    """특정 step_id로 점프"""
...
class ChangeSubTaskException(FlowControlException):
    """서브 태스크 변경 예외"""
...
class SubTaskHaltException(FlowControlException):
    """서브 태스크 실행 중단 예외"""
