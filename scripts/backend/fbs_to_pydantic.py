...
def parse_schema(path: Path) -> SchemaUnit:
    ...
    unions: list[UnionDef] = []
    for um in re.finditer(r"\bunion\s+([A-Za-z_]\w*)\s*\{(.*?)\}", raw, flags=re.DOTALL):
        name = um.group(1)
        body = um.group(2)
    ...
    blocks: list[BlockDef] = []
    for bm in re.finditer(r"\b(table|struct)\s+([A-Za-z_]\w*)\s*\{(.*?)\}", raw, flags=re.DOTALL):
        kind = bm.group(1)
        name = bm.group(2)
        body = bm.group(3)
        fields: list[FieldDef] = []
...
def convert_type(fbs_type: str) -> tuple[str, str | None]:
    ...
    if vec is not None:
        ...
        return f"list[{py_base}]", f"Field(default_factory=list, min_length={fixed_len}, max_length={fixed_len})"

    py_scalar = SCALAR_MAP.get(fbs_type)
    if py_scalar:
        return py_scalar, None

    return pd_name(fbs_type.split(".")[-1]), None
...
def render_module(unit: SchemaUnit) -> str:
    lines: list[str] = []
    lines.append("# AUTO-GENERATED by scripts/fbs_to_pydantic.py. DO NOT EDIT.")
    ...
    include_modules = sorted({snake_case(Path(x).stem) + "_models" for x in unit.includes})
    if include_modules:
    ...
    for block in unit.blocks:
        ...
        for field in block.fields:
            ...
            if fixed_meta is not None:
                ...
                field_expr = f"Field({', '.join(field_args)})" if field_args else "Field()"
...
def update_init_files_recursively(output_root: Path):
    all_dirs = sorted([p for p in output_root.rglob("*") if p.is_dir()], key=lambda p: len(p.parts), reverse=True)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Generate pydantic models from FlatBuffers schemas")
    parser.add_argument(
        "--input",
        type=Path,
