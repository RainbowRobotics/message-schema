...
public:
    ...
    int FIXED_SEND_IMG_HEIGHT = 750;
    int FIXED_SEND_IMG_WIDTH = 320;
...
Q_SIGNALS:
    ...
    void signal_move_goal(double time, QString node_id, int preset, QString method, Eigen::Vector3d offset_val);
    void signal_move_docking(double time, QString method, double offset_x, double offset_y, double offset_th);
    ...
    void signal_sync_infos(double time, QString dir,QString user,QString ip, QString password);
    void signal_adjustment_config(double time, QString config, QString val);

    void signal_comm_ui_view_control(double time, QString val);
    void signal_comm_ui_view_type(double time, QString val);
...
public Q_SLOTS:
    ...
    void slot_move_goal(double time, QString node_id, int preset, QString method, Eigen::Vector3d offset_val);
    void slot_move_docking(double time, QString method, double offset_x, double offset_y, double offset_th);
    ...
    void slot_sync_infos(double time, QString dir,QString user,QString ip, QString password);
    void slot_adjustment_config(double time, QString config, QString val);

    // send slots
    void send_status();
    void send_img(const cv::Mat& img);
    ...
    void send_move_docking_response(QString method, double offset_x, double offset_y, double offset_th, QString result, QString message);
    ...
    void send_sync_infos_response(QString result, QString message);
    void send_adjustment_config_response(QString result, QString message);
};
