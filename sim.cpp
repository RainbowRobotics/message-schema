...
void SIM::a_loop()
{
    ...
    while(a_flag)
    {
        ...
        double wz1 = saturation(mobile->wz0, -config->MOTOR_LIMIT_W, config->MOTOR_LIMIT_W);
        ...
        if(wz1-wz0 > 0.001)
        {
            double dwz = config->MOTOR_LIMIT_W_ACC*dt;
            wz = wz0 + dwz;
            if(wz > wz1)
            {
                wz = wz1;
        ...
        for(size_t p = 0; p < res_idxs.size(); p += 4)
        {
            ...
            Eigen::Vector3d _P = tf_inv.block(0,0,3,3)*P + tf_inv.block(0,3,3,1);
            pts.push_back(_P);
