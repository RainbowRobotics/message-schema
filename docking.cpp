...
//            qDebug() <<"dist is " <<d ;
    ...
    qDebug() << "filtered_Clusters size is " << filtered_clusters.size();
    if(!sizefilter(filtered_clusters,docking_clust))
    {
        qDebug() << " size filter false";
        return false;
    }
    
    clusters_queue.push(docking_clust);

//    qDebug() << "docking_clust sisze" << docking_clust.size();
    if(clusters_queue.size() > 10)
    {
        ...
        qDebug() << "cur_frm center" << frm0_center0(0) << frm0_center0(1);
        qDebug() << "vfrm center" << frm1_center(0) << frm1_center(1) << frm1_center(2);

        double err = Vfrm_icp(cur_frm, Vfrm, dock_tf);
        qDebug() << "q10 icp err" <<err;
...
void DOCKING::a_loop()
{
    ...
    while(a_flag){
        ...
        if(fsm_state == DOCKING_FSM_POINTDOCK)
        {
            ...
            // pointdock 에서 vmark가 안되는 순간에 멈추는기능 추가해야함
            find_Vmark();
            qDebug() << "Point dock";
            if(path_flag)
            {
...
//            if(odom_v_dock < config->DOCKING_POINTDOCK_MARGIN)
//            {
//                cmd_v = 0.03;
...
//            {
//                mobile->move(0,0,0);
//                //To do-- CHARGE STATE CHECKING
//                qDebug() << "Docking Process Complete";
//            }
...
//        }
//        qDebug() << " size is " << size ;
//        if(size >=0.40 && size <=0.43)
//        {
//           candidate.push_back(clusters);
//        }
...
//    }
    ...
    if(candidate.size() == 1)
    {
//        for(auto&t : candidate[0])
//        {
//            debug_frame.push_back(t);
//        }
        out = candidate[0];
        return true;
    }
...
//            for(auto& point : candi)
//            {
//                sum_x += point.x();
//                sum_y += point.y();
//            }
//            float c_x = sum_x / candi.size();
//            float c_y = sum_y / candi.size();
//            float dist =std::sqrt(c_x*c_x + c_y*c_y);
...
//        else
//        {
//            printf("[DOCKING] code something wrong (failed)\n");
//            return false;
//        }
