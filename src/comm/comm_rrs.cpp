...
void ERROR_MANAGER::logError(ErrorCause cause, ErrorContext context, const QString& additional_info)
{
    ...
    if(info.level == "critical")
    {
        ...
        log_critical("{}", log_message.toStdString());
    }
    else if(info.level == "error")
    {
        //spdlog::error("[RRS] {}", log_message.toStdString());
        log_error("{}", log_message.toStdString());
    }
    else if(info.level == "warn")
    {
        //spdlog::warn("[RRS] {}", log_message.toStdString());
        log_warn("{}", log_message.toStdString());
    }
    else
    {
        //spdlog::info("[RRS] {}", log_message.toStdString());
        log_info("{}", log_message.toStdString());
    }
}
...
void COMM_RRS::recv_profile_move(std::string const& name, sio::message::ptr const& data, bool hasAck, sio::message::list &ack_resp)
{
    if(data && data->get_flag() == sio::message::flag_object)
    {
        ...
        msg.command = get_json(data, "command"); // "linearXMove", "circularMove" , "ratateMove", "linearStop"
        msg.time = get_json(data, "time").toDouble() / 1000;
    ...
    if(command == "linearXMove")
    {
        ...
        else
        {
            ...
            QTimer::singleShot(t*1000, [this, msg]() mutable
            {
                ...
                send_profile_move_response(msg);
                //qDebug() << "send_profile_move_response";
                return;
            });
