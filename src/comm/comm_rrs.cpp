  ...
  , dctrl(nullptr)
{
    ...
    BIND_EVENT(sock, "profileMove",     std::bind(&COMM_RRS::recv_profile_move,        this, _1, _2, _3, _4));
    ...
    connect(this, &COMM_RRS::signal_profile_move,    this, &COMM_RRS::slot_profile_move);
...
void COMM_RRS::init()
{
    ...
    if(!config->get_use_rrs())
    {
        ...
        log_info("use_rrs = false, skip connect");
        return;
    }
...
void COMM_RRS::recv_move(std::string const& name, sio::message::ptr const& data, bool hasAck, sio::message::list &ack_resp)
{
    if(data && data->get_flag() == sio::message::flag_object)
    {
        ...
        log_info("recv, move, command: {}, time: {}",msg.command.toStdString(),msg.time);
...
void COMM_RRS::recv_localization(std::string const& name, sio::message::ptr const& data, bool hasAck, sio::message::list &ack_resp)
{
    if(data && data->get_flag() == sio::message::flag_object)
    {
        ...
        log_info("recv, loc, command: {}, time: {}",msg.command.toStdString(),msg.time);
...
void COMM_RRS::recv_load(std::string const& name, sio::message::ptr const& data, bool hasAck, sio::message::list &ack_resp)
{
    if(data && data->get_flag() == sio::message::flag_object)
    {
        ...
        log_info("recv, load, command: {}, time: {}",msg.command.toStdString(),msg.time);
...
void COMM_RRS::recv_randomseq(std::string const& name, sio::message::ptr const& data, bool hasAck, sio::message::list &ack_resp)
{
    if(data && data->get_flag() == sio::message::flag_object)
    {
        ...
        log_info("recv, randomseq, command: {}, time: {}",msg.command.toStdString(),msg.time);
...
void COMM_RRS::recv_mapping(std::string const& name, sio::message::ptr const& data, bool hasAck, sio::message::list &ack_resp)
{
    if(data && data->get_flag() == sio::message::flag_object)
    {
        ...
        log_info("recv, mapping, command: {}, time: {}",msg.command.toStdString(),msg.time);
...
void COMM_RRS::recv_dock(std::string const& name, sio::message::ptr const& data, bool hasAck, sio::message::list &ack_resp)
{
    if(data && data->get_flag() == sio::message::flag_object)
    {
        ...
        log_info("recv, dock, command: {}, time: {}",msg.command.toStdString(),msg.time);
...
void COMM_RRS::recv_safety_request(std::string const& name, sio::message::ptr const& data, bool hasAck, sio::message::list &ack_resp)
{
    if(data && data->get_flag() == sio::message::flag_object)
    {
        ...
        log_info("recv, field set filed: {}", msg.set_field);
        ...
        log_info("recv, field_set, command: {}, time: {}",msg.command.toStdString(),msg.time);
...
void COMM_RRS::recv_obs_box_request(std::string const& name, sio::message::ptr const& data, bool hasAck, sio::message::list &ack_resp)
{
    if(data && data->get_flag() == sio::message::flag_object)
    {
        ...
        log_info("recv_obs_box_request, command: {}, min_z: {}, max_z: {}, map_range: {}, time: {}",
                 msg.command.toStdString(), msg.obs_box_min_z, msg.obs_box_max_z, msg.obs_box_map_range, msg.time);
        Q_EMIT signal_obs_box_setting(msg);
    }
}


void COMM_RRS::recv_profile_move(std::string const& name, sio::message::ptr const& data, bool hasAck, sio::message::list &ack_resp)
{
    if(data && data->get_flag() == sio::message::flag_object)
    {
        // parsing
        DATA_PROFILE_MOVE msg;
        msg.command = get_json(data, "command"); // "linearXMove", "linearYMove","circularMove" , "ratateMove", "linearStop" 
        msg.time = get_json(data, "time").toDouble() / 1000;

        msg.target = get_json(data, "target").toDouble(); // m or deg
        msg.speed = get_json(data, "speed").toDouble(); // m/s or deg/s
        msg.dir = get_json(data, "direction"); // "right", "left"
...
void COMM_RRS::recv_config_request(std::string const& name, sio::message::ptr const& data, bool hasAck, sio::message::list &ack_resp)
{
    ...
    if(command == "getDriveParam" || command == "param_set" || command == "setParam")
    {
                ...
                if(params_array)
                {
                    ...
                    for(const auto& param_value : params_array->get_vector())
                    {
                        ...
                        else
                        {
                            ...
                            log_error("error : param is not an object, type: {}", static_cast<int>(param_value->get_flag()));
        ...
        else
        {
            ...
            log_error("params field not found");
        }
        ...
        log_info("recv, config_request, command: {}, time: {}",msg.command.toStdString(),msg.time);
...
void COMM_RRS::recv_view_lidar_on_off(std::string const& name, sio::message::ptr const& data, bool hasAck, sio::message::list &ack_resp)
{
    if(data && data->get_flag() == sio::message::flag_object)
    {
        ...
        log_info("recv, lidar_on_off, command: {}, time: {}",msg.command.toStdString(),msg.time);
...
void COMM_RRS::recv_view_path_on_off(std::string const& name, sio::message::ptr const& data, bool hasAck, sio::message::list &ack_resp)
{
    if(data && data->get_flag() == sio::message::flag_object)
    {
        ...
        log_info("recv, path_on_off, command: {}, time: {}",msg.command.toStdString(),msg.time);
...
void COMM_RRS::recv_led(std::string const& name, sio::message::ptr const& data, bool hasAck, sio::message::list &ack_resp)
{
    if(data && data->get_flag() == sio::message::flag_object)
    {
        ...
        log_info("recv, led_on_off, command: {}, time: {}",msg.command.toStdString(),msg.time);
...
void COMM_RRS::recv_motor(std::string const& name, sio::message::ptr const& data, bool hasAck, sio::message::list &ack_resp)
{
    if(data && data->get_flag() == sio::message::flag_object)
    {
        ...
        log_info("recv, motor_on_off, command: {}, time: {}",msg.command.toStdString(),msg.time);
...
void COMM_RRS::recv_path(std::string const& name, sio::message::ptr const& data, bool hasAck, sio::message::list &ack_resp)
{
    if(data && data->get_flag() == sio::message::flag_object)
    {
        ...
        log_info("recv, path, command: {}, path: {}, time: {}",msg.command.toStdString(), msg.path.toStdString(), msg.time);
...
void COMM_RRS::recv_vobs(std::string const& name, sio::message::ptr const& data, bool hasAck, sio::message::list &ack_resp)
{
    if(data && data->get_flag() == sio::message::flag_object)
    {
        ...
        log_info("recv, vobs, command: {}, vobs_r: {}, vobs_c: {}, time: {}",msg.command.toStdString(), msg.vobs_robots.toStdString(), msg.vobs_clousers.toStdString(), msg.time);
...
void COMM_RRS::recv_foot(const std::string& name, const sio::message::ptr& data, bool hasAck, sio::message::list& ack_resp)
{
    ...
    if(data && data->get_flag() == sio::message::flag_object)
    {
        ...
        _main->set_temperature_value(temperature_msg.temperature_value);
...
void COMM_RRS::recv_safety_io_set(const std::string& name, const sio::message::ptr& data, bool hasAck, sio::message::list& ack_resp)
{
    ...
    if(config->set_debug_comm_rrs())
    {
        ...
        log_info("recv_safety io called");
    }
    ...
    if(data && data->get_flag() == sio::message::flag_object)
    {
        ...
        if(config->set_debug_comm_rrs())
        {
            ...
            log_info("time: {}",time_str.toStdString());
        }
        ...
        if(!safetyio_msg || safetyio_msg->get_flag() != sio::message::flag_object)
        {
            ...
            if(config->set_debug_comm_rrs())
            {
                ...
                log_info("'foot' field missing or not object");
            }
            return;
        }
...
void COMM_RRS::send_move_status()
{
    if(!is_connected || !ctrl || !mobile || !unimap || !dctrl)
    {
        ...
        log_info("is_connected : {}", (int)is_connected.load());
...
void COMM_RRS::send_mapping_cloud()
{
    ...
    log_info("send_mapping_cloud called");
...
void COMM_RRS::slot_move(DATA_MOVE msg)
{
    ...
    else if(command == "target")
    {
        ...
        if(method == "pp")
        {
            ...
            {
                msg.result = "reject";
                msg.message = ERROR_MANAGER::instance()->getErrorMessage(ERROR_MANAGER::MOVE_TARGET_OUT_RANGE, ERROR_MANAGER::MOVE_TARGET);
                msg.bat_percent = bat_percent;

                ERROR_MANAGER::instance()->logError(ERROR_MANAGER::MOVE_TARGET_OUT_RANGE, ERROR_MANAGER::MOVE_TARGET);
                send_move_response(msg);
                return;
            }
            ...
            {
                msg.result = "reject";
                msg.message = ERROR_MANAGER::instance()->getErrorMessage(ERROR_MANAGER::MOVE_TARGET_OCCUPIED, ERROR_MANAGER::MOVE_TARGET);
                msg.bat_percent = bat_percent;

                ERROR_MANAGER::instance()->logError(ERROR_MANAGER::MOVE_TARGET_OCCUPIED, ERROR_MANAGER::MOVE_TARGET);
                send_move_response(msg);
                return;
            }
            ...
            {
                msg.result = "reject";
                msg.message = ERROR_MANAGER::instance()->getErrorMessage(ERROR_MANAGER::SYS_MULTI_MODE_LIMIT, ERROR_MANAGER::MOVE_TARGET);
                msg.bat_percent = bat_percent;

                ERROR_MANAGER::instance()->logError(ERROR_MANAGER::SYS_MULTI_MODE_LIMIT, ERROR_MANAGER::MOVE_TARGET);
                send_move_response(msg);
                return;
            }
        ...
        else
        {
            ...
            log_error("system not suppoerted");
            msg.result = "reject";
    ...
    else if(command == "goal" || command == "change_goal")
    {
        ...
        if(method == "pp")
        {
            ...
            {
                msg.result = "reject";
                msg.message = ERROR_MANAGER::instance()->getErrorMessage(ERROR_MANAGER::SYS_NOT_SUPPORTED, ERROR_MANAGER::MOVE_GOAL);
                msg.bat_percent = bat_percent;

                ERROR_MANAGER::instance()->logError(ERROR_MANAGER::SYS_NOT_SUPPORTED, ERROR_MANAGER::MOVE_GOAL);
                send_move_response(msg);
                return;
            }
...
void COMM_RRS::slot_mapping(DATA_MAPPING msg)
{
    ...
    else if(command == "save")
    {
        ...
        if(_main)
        {
            ...
            {
                msg.result = "fail";
                msg.message = ERROR_MANAGER::instance()->getErrorMessage(ERROR_MANAGER::MAP_COPY_FAILED, ERROR_MANAGER::MAPPING_SAVE);

                ERROR_MANAGER::instance()->logError(ERROR_MANAGER::MAP_COPY_FAILED, ERROR_MANAGER::MAPPING_SAVE);
                send_mapping_response(msg);
...
void COMM_RRS::slot_load(DATA_LOAD msg)
{
    ...
    if(command == "mapload")
    {
        ...
        if(!load_dir.isNull())
        {
            ...
            {
                msg.result = "reject";
                msg.message = ERROR_MANAGER::instance()->getErrorMessage(ERROR_MANAGER::MAP_INVALID_PATH, ERROR_MANAGER::LOAD_MAP);

                ERROR_MANAGER::instance()->logError(ERROR_MANAGER::MAP_INVALID_PATH, ERROR_MANAGER::LOAD_MAP);
            ...
            {
                msg.result = "fail";
                msg.message = ERROR_MANAGER::instance()->getErrorMessage(ERROR_MANAGER::MAP_LOAD_FAILED, ERROR_MANAGER::LOAD_MAP);

                ERROR_MANAGER::instance()->logError(ERROR_MANAGER::MAP_LOAD_FAILED, ERROR_MANAGER::LOAD_MAP);
                send_load_response(msg);
            }
        }
    }
    else if(command == "topoload")
    {
        ...
        if(config->get_use_sim() || ms.motor_stop_state ==1)
        {
            ...
            if(!load_dir.isNull())
            {
                ...
                {
                    msg.result = "reject";
                    msg.message = ERROR_MANAGER::instance()->getErrorMessage(ERROR_MANAGER::MAP_INVALID_PATH, ERROR_MANAGER::LOAD_TOPO);

                    ERROR_MANAGER::instance()->logError(ERROR_MANAGER::MAP_INVALID_PATH, ERROR_MANAGER::LOAD_TOPO);
                    send_load_response(msg);
                    return;
                }
                ...
                else
                {
                    ...
                    msg.result = "fail";
                    msg.message = ERROR_MANAGER::instance()->getErrorMessage(ERROR_MANAGER::MAP_TOPO_LOAD_FAILED, ERROR_MANAGER::LOAD_TOPO);

                    ERROR_MANAGER::instance()->logError(ERROR_MANAGER::MAP_TOPO_LOAD_FAILED, ERROR_MANAGER::LOAD_TOPO);
                    send_load_response(msg);
                }
            }
    ...
    else if(command == "configload")
    {
        ...
        msg.result = "reject";
        msg.message = ERROR_MANAGER::instance()->getErrorMessage(ERROR_MANAGER::SYS_NOT_SUPPORTED, ERROR_MANAGER::LOAD_CONFIG);

        ERROR_MANAGER::instance()->logError(ERROR_MANAGER::SYS_NOT_SUPPORTED, ERROR_MANAGER::LOAD_CONFIG);
        send_load_response(msg);
    }
}
...
void COMM_RRS::slot_localization(DATA_LOCALIZATION msg)
{
    ...
    if(command == "semiautoinit")
    {
        ...
        {
            msg.result = "reject";
            msg.message = ERROR_MANAGER::instance()->getErrorMessage(ERROR_MANAGER::LOC_SENSOR_ERROR, ERROR_MANAGER::LOC_SEMI_AUTO);

            ERROR_MANAGER::instance()->logError(ERROR_MANAGER::LOC_SENSOR_ERROR, ERROR_MANAGER::LOC_SEMI_AUTO);
            send_localization_response(msg);
            return;
        }
        ...
        if(loc)
        {
            ...
            {
                msg.result = "fail";
                msg.message = ERROR_MANAGER::instance()->getErrorMessage(ERROR_MANAGER::LOC_INIT_FAILED, ERROR_MANAGER::LOC_SEMI_AUTO);
                ERROR_MANAGER::instance()->logError(ERROR_MANAGER::LOC_INIT_FAILED, ERROR_MANAGER::LOC_SEMI_AUTO);
                send_localization_response(msg);
                return;
            }
        }
    ...
    else if(command == "init")
    {
        ...
        {
            msg.result = "reject";
            msg.message = ERROR_MANAGER::instance()->getErrorMessage(ERROR_MANAGER::LOC_SENSOR_ERROR, ERROR_MANAGER::LOC_INIT);

            ERROR_MANAGER::instance()->logError(ERROR_MANAGER::LOC_SENSOR_ERROR, ERROR_MANAGER::LOC_INIT);
            send_localization_response(msg);
            return;
        }
        ...
        if(loc)
        {
            ...
            msg.result = "fail";
            msg.message = ERROR_MANAGER::instance()->getErrorMessage(ERROR_MANAGER::LOC_INIT_FAILED, ERROR_MANAGER::LOC_INIT);
            ERROR_MANAGER::instance()->logError(ERROR_MANAGER::LOC_INIT_FAILED, ERROR_MANAGER::LOC_INIT);
            send_localization_response(msg);
            return;
    ...
    else if(command == "start")
    {
        ...
        if(loc)
        {
            ...
            msg.result = "fail";
            msg.message = ERROR_MANAGER::instance()->getErrorMessage(ERROR_MANAGER::LOC_INIT_FAILED, ERROR_MANAGER::LOC_START);
            ERROR_MANAGER::instance()->logError(ERROR_MANAGER::LOC_INIT_FAILED, ERROR_MANAGER::LOC_START);
            send_localization_response(msg);
            return;
        }
        else
    ...
    else if(command == "stop")
    {
        ...
        {
            msg.result = "fail";
            msg.message = ERROR_MANAGER::instance()->getErrorMessage(ERROR_MANAGER::LOC_INIT_FAILED, ERROR_MANAGER::LOC_STOP);
            ERROR_MANAGER::instance()->logError(ERROR_MANAGER::LOC_INIT_FAILED, ERROR_MANAGER::LOC_STOP);
            send_localization_response(msg);
            return;
        }
    }
...
void COMM_RRS::slot_profile_move(DATA_PROFILE_MOVE msg)
{
    const QString command = msg.command;
    ...
    if(command == "linearXMove")
    {
        ...
        else
        {
            ...
            QTimer::singleShot(t*1000, [this, msg]() mutable
            {
                ...
                msg.result = "success";
                msg.message = "";
                msg.remain_dist = res_linear_remain_dist;
    ...
    else if(command == "circularMove")
    {
        ...
        else
        {
            ...
            QTimer::singleShot(t*1000, [this, msg]() mutable
            {
                ...
                msg.result = "success";
                msg.message = "";
                msg.remain_dist = res_circular_remain_dist;
...
void COMM_RRS::slot_software_update(DATA_SOFTWARE msg)
{
    ...
    if(ms.charge_state == 0 || ms.motor_stop_state >= 1)
    {
        ...
        {
            msg.result = "false";
            msg.message = ERROR_MANAGER::instance()->getErrorMessage(ERROR_MANAGER::BAT_NOT_CHARGING, ERROR_MANAGER::SOFTWARE_UPDATE);

            ERROR_MANAGER::instance()->logError(ERROR_MANAGER::BAT_NOT_CHARGING, ERROR_MANAGER::SOFTWARE_UPDATE);
        ...
        {
            msg.result = "false";
            msg.message = ERROR_MANAGER::instance()->getErrorMessage(ERROR_MANAGER::SAFETY_EMO_RELEASED, ERROR_MANAGER::SOFTWARE_UPDATE);

            ERROR_MANAGER::instance()->logError(ERROR_MANAGER::SAFETY_EMO_RELEASED, ERROR_MANAGER::SOFTWARE_UPDATE);
            send_software_update_response(msg);
        }
        return;
    }
    ...
    {
        msg.result = "false";
        msg.message = ERROR_MANAGER::instance()->getErrorMessage(ERROR_MANAGER::SYS_PROCESS_START_FAILED, ERROR_MANAGER::SOFTWARE_UPDATE);

        ERROR_MANAGER::instance()->logError(ERROR_MANAGER::SYS_PROCESS_START_FAILED, ERROR_MANAGER::SOFTWARE_UPDATE);
        send_software_update_response(msg);
        return;
    }
    ...
    {
        msg.result = "false";
        msg.message = ERROR_MANAGER::instance()->getErrorMessage(ERROR_MANAGER::SYS_PROCESS_FINISH_FAILED, ERROR_MANAGER::SOFTWARE_UPDATE);

        ERROR_MANAGER::instance()->logError(ERROR_MANAGER::SYS_PROCESS_FINISH_FAILED, ERROR_MANAGER::SOFTWARE_UPDATE);
        send_software_update_response(msg);
        return;
    }
...
void COMM_RRS::slot_foot(DATA_FOOT msg)
{
    ...
    if(msg.state == static_cast<int>(FootState::DONE) && msg.is_down == true)
    {
...
void COMM_RRS::send_profile_move_response(const DATA_PROFILE_MOVE& msg)
{
    if(!is_connected)
    {
        return;
    ...
    obj["result"] = msg.result;
    obj["message"] = msg.message;
    obj["remain_dist"] = QString::number(msg.remain_dist, 'f', 3);
    obj["meassured_dist"] = QString::number(msg.meassured_dist, 'f', 3);
    obj["time"] = QString::number((long long)(msg.time*1000), 10);

    QJsonDocument doc(obj);
    sio::message::ptr res = sio::string_message::create(doc.toJson().toStdString());
    io->socket()->emit("profileMoveResponse", res);
