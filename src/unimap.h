...
using node_table = multi_index_container<NODE, indexed_by<ordered_unique<tag<by_id>,   member<NODE, QString, &NODE::id>>,
                                                          ordered_unique<tag<by_name>, member<NODE, QString, &NODE::name>>>>;


struct UNIMAP_INFO
{
    static constexpr int knn_search_num = 10;

    static constexpr int max_candidates_size = 20;
    static constexpr double search_radius_weight = 5.0;
};
...
public:
    ...
    NODE get_goal_node(Eigen::Vector3d pos);                                               // get goal node(goal,init,station) id nearest input pos(x,y,th)
    ...
    std::vector<NODE> get_init_candidates_id(QString type, std::vector<QString> name_list); // get init candidates for semiauto-init
    ...
    std::vector<NODE> get_nodes();                                  // get nodes (shared ptr)
