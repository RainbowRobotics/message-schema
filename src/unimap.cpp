        ...
        // set fast find nodes instance
        {
            ...
            for(size_t i = 0; i < links.size(); i++)
            {
                ...
                // debug
                {
                    ...
                    spdlog::info("[UNIMAP] special_links size = {}", special_links.size());
                    ...
                    for(size_t i = 0; i < special_links.size(); i++)
                    {
                        ...
                        spdlog::info("[UNIMAP][SPECIAL_LINK][{}]", i);
                        spdlog::info("  st_id = {}, ed_id = {}, info = {}", s.st_id.toStdString(), s.ed_id.toStdString(), s.info.toStdString());
                        spdlog::info("  length = {:.3f}", s.length);
                        spdlog::info("  st  = ({:.3f}, {:.3f}, {:.3f})", s.st.x(), s.st.y(), s.st.z());
                        spdlog::info("  ed  = ({:.3f}, {:.3f}, {:.3f})", s.ed.x(), s.ed.y(), s.ed.z());
                        spdlog::info("  mid = ({:.3f}, {:.3f}, {:.3f})", s.mid.x(), s.mid.y(), s.mid.z());
                    }
                }
            }
