...
bool UNIMAP::load_3d()
{
    ...
    // update
    {
        ...
        // set plot storage
        map_3d_pts      = std::make_unique<std::vector<Eigen::Vector3d>>(pts);
        map_3d_normal   = std::make_unique<std::vector<Eigen::Vector3d>>(nor);
        map_3d_reflects = std::make_unique<std::vector<double>>(reflects);
    }
