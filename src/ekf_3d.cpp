...
void EKF_3D::init(const Eigen::Matrix4d& tf)
{
    ...
    Eigen::Vector6d x_hat = TF_to_ZYX(tf);
    ...
    printf("[EKF] init (%.3f, %.3f, %.3f, %.3f, %.3f, %.3f )\n",
...
void EKF_3D::predict(const Eigen::Matrix4d& odom_tf)
{
    ...
    x_bar(3) = phi;
    x_bar(4) = theta;
    x_bar(5) = std::atan2(std::sin(psi + delta_th), std::cos(psi + delta_th));

    // jacobian F_k
    Eigen::Matrix6d F_k = Eigen::Matrix6d::Identity();
    F_k(0,3) = (cos_psi*sin_theta*cos_phi + sin_psi*sin_phi) * delta_y;
    F_k(0,4) = (sin_psi*sin_theta*cos_phi - cos_psi*sin_phi) * delta_y;
    F_k(0,5) = (cos_theta*cos_phi) * delta_y;

    F_k(1,3) = (-cos_psi*sin_theta) * delta_x + cos_psi*cos_theta*sin_phi * delta_y;
    F_k(1,4) = (-sin_psi*sin_theta) * delta_x + (sin_psi*cos_theta*sin_phi) * delta_y;
    F_k(1,5) = (-cos_theta) * delta_x + (-sin_theta*sin_phi) * delta_y;

    F_k(2,3) = (-sin_psi*cos_theta) * delta_x + (-sin_psi*sin_theta*sin_phi - cos_psi*cos_phi) * delta_y;
    F_k(2,4) = (cos_psi*cos_theta) * delta_x + (cos_psi*sin_theta*sin_phi - sin_psi*cos_phi) * delta_y;
    F_k(2,5) = 0.0;
