...
GLS611::GLS611(QObject *parent)
    : QObject{parent}
    , reconnect_timer(this)
{
    connect(&socket_bqr, &QTcpSocket::connected, this, &GLS611::connected);
    connect(&socket_bqr, &QTcpSocket::disconnected, this, &GLS611::disconnected);
...
void GLS611::process_data(QString data)
{
    data.remove(0,1);
...
void GLS611::connected()
{
    is_connected = true;
    connect(&socket_bqr, &QTcpSocket::readyRead, this, &GLS611::readyread);
    logger->write_log("[BQR] connected", "Green", true, false);
}

void GLS611::disconnected()
{
    is_connected = false;
...
TIME_POSE_ID GLS611::get_cur_tpi()
{
    mtx.lock();
